...
const {
  ...
  createNewActivity,
  getAllActivities,
  findActivity } = require("../Controllers/getCounrties")
...
// Ruta Traigo todas las actividades o alguna por query 
router.get("/activities", async (req, res) => { 
    const { name } = req.query;
    let activities ;
    try {
        if (name) { 
            activities = await findActivity(name.trim());
            res.status(200).json({
                status: true,
                result: activities
            });
            ...
            if (name.trim() === "") {
                ...
                if (regex_FullText.test(name)) {
                    ...
                    if (Countrys.length == 0) {
                        res.status(500).json({message: `No se encontro ningun pais con el atributo ${name}`})
